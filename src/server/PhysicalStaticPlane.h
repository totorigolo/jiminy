...
class PhysicalStaticPlane : public Physical3DPrimitive
{
public:
    PhysicalStaticPlane(const btVector3 &planeNormal, const btScalar planeConstant,
                        const btTransform &startTrans = btTransform::getIdentity(),
                        const btTransform &centerOfMassOffset = btTransform::getIdentity());

    virtual ~PhysicalStaticPlane();
