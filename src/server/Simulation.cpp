...
//            base->mB2Body->SetLinearVelocity(b2Vec2(base->mB2Body->GetLinearVelocity().x, 0));
        ...
        while (isSimulating() && window.isOpen())
        {
            ...
            while (window.pollEvent(event))
            {
                ...
                {
                    view.setSize(
                            u2f(window.getSize()) * sf::Vector2f(view.getViewport().width, view.getViewport().height));
                ...
                {
                    createB2Circle(mB2World, convert(window.mapPixelToCoords(sf::Mouse::getPosition(window), view)),
                                   2.f);
                }
